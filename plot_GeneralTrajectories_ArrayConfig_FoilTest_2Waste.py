...
parser.add_argument("--directoryWaste2", dest='directoryWaste2', default="output5", help="directory to put graphs into")
...
fileNameArrayWaste2=plottingSettingsDictionary.getArray("fileNameArrayWaste2")
fileNameArrayClosed=plottingSettingsDictionary.getArray("fileNameArrayClosed")
fileNamePrefix=["emmit_beg_","emmit_end_"]
coordinateArrayWaste={}
coordinateArrayWaste2={}
...
theDirectoryWaste2=args.directoryWaste2
...
for prefixFileName in fileNamePrefix:
	for currentFileName in fileNameArrayInjection:
		...
		openedFile=open("%s/%s%s.txt"%(theDirectoryInjection,prefixFileName,currentFileName),'r')
...
for prefixFileName in fileNamePrefix:
	for currentFileName in fileNameArrayClosed:
		#print "%s/%s%s%d.txt"%(theDirectoryClosed,prefixFileName,currentFileName,stripper1CurrentLocation)
		openedFile=open("%s/%s%s.txt"%(theDirectoryClosed,prefixFileName,currentFileName),'r')
...
for prefixFileName in fileNamePrefix:
	for currentFileName in fileNameArrayWaste:
		...
		openedFile=open("%s/%s%s.txt"%(theDirectoryWaste,prefixFileName,currentFileName),'r')
...
#build drawing array
for prefixFileName in fileNamePrefix:
	for currentFileName in fileNameArrayWaste2:
		#print "%s/%s%s%d.txt"%(theDirectory,prefixFileName,currentFileName,stripper1CurrentLocation)
		openedFile=open("%s/%s%s.txt"%(theDirectoryWaste2,prefixFileName,currentFileName),'r')
		...
		for line in lines:
			...
			xCoord="hi2"
			xCoord=token
			trackingArrayWaste2.append([float(xCoord),float(sCoord)])
			coords.append([float(xCoord),float(sCoord)])
		coordinateArrayWaste2[prefixFileName+currentFileName] =coords
#print "testPrint"
trackingArrayWaste2=sorted(trackingArrayWaste2,key=lambda x: (x[1]))
...
referenceLengthWaste2=trackingArrayWaste2[0][1]
...
referenceLengthClosed=trackingArrayClosed[len(trackingArrayClosed)-1][1] - trackingArrayWaste[len(trackingArrayWaste)-1][1]
#referenceLengthClosed=trackingArrayClosed[0][1]
#print "CHECK"
...
driftTrackArrayWaste2=[]	
for index in range(len(trackingArrayWaste2)):
	#print trackingArray[index]
	
	if index!=len(trackingArrayWaste2)-1:
	#if index==5:
		driftTrack=None
		firstX=1+0.5*trackingArrayWaste2[index][0]/theMax
		secondX=1+0.5*trackingArrayWaste2[index+1][0]/theMax
		driftTrack=TLine((trackingArrayWaste2[index][1]-referenceLengthWaste2)/Length,midLine*firstX,(trackingArrayWaste2[index+1][1]-referenceLengthWaste2)/Length,midLine*secondX)
		driftTrack.SetNDC()
		driftTrack.SetLineColor(kMagenta+2)
		driftTrackArrayWaste2.append(driftTrack)
		#driftTrack.Draw("same")
...
for drift in driftTrackArrayWaste:
	drift.Draw("same")
for drift in driftTrackArrayWaste2:
	drift.Draw("same")	
for drift in driftTrackArrayClosed:
	drift.Draw("same")	

theCanvas.Print("%s/tracking_%s.%s"%(args.directoryOutput,tokenToFind[0],args.imageType))
...
#print referenceLengthClosed
#print referenceLengthClosed2
